...
class FF_CLIENT(threading.Thread):
    ...
    def GenResponsMsg(self, Msg):
        ...
        1: 1,
        2: {
            1: 11524993139,#bot account id
            2: 3074977289,#clan id
            3: 1,
            4: str(Msg),
    ...
    def joinclanchat(self):
        fields = {
            1: 3,
            2: {
                1: 3074977289,#clan id
                2: 1,
                4: str("QcpQpbn664hzaR2XEmEaOA"),#clan key
            }
        }
        packet = create_protobuf_packet(fields)
        packet = packet.hex()
                ...
                if "8" in parsed_data["5"]["data"] and "data" in parsed_data["5"]["data"]["8"]:
                    ...
                    if uexmojiii == "DefaultMessageWithKey":
                        ...
                        clients.send(
                            self.GenResponsMsg(
                                f"""[C][B]
مرحبًا بكم في كلان [00ff00]『σм』ㅤTEAMㅤ[ffffff]!

[C][B]
إذا أردت معرفة الأوامر، فقط أرسل: [00ff00]🗿/🗿help🗿[ffffff]

[C][B]
تنبيه! البوت لا يشتغل إلا لمن لديهم تاغ الكلان: [00ff00]『σм』[ffffff]

[ffffff]للحصول على شعار  لكي يعمل معك البوت ارسل: [00ff00]/sl🇴🇲og
"""
                            )
                        )
                else:
...
####################################
            ...
            if "1200" in data.hex()[0:4] and b"/snd/" in data:
                try:
                    ...
                    if "『σм』" not in sender_name:
	                    clients.send(
            self.GenResponsMsg(
                f"[C][B][FF0000]Access Denied!\n[ffffff]Only users with [『σм』] in their name can use this command."
            )
        )
                    else:
...
[cُ][bَ][00E5FF]
/h🇴🇲elp          
[FFFFFF]عرض جميع الأوامر

[cُ][bَ][00E676]
/🇴🇲add id time    
[FFFFFF]ارسال البوت لشخص[الادمن فقط]
...
[cُ][bَ][FF00FF]
/v🇴🇲ip       
[FFFFFF] VIP (تعريفات المقبرة)

[cُ][bَ][FF9100]
/r🇴🇲m msg        
[FFFFFF]كتابة رسالة باسمك (استبدل msg بنصك)
"""
	                clients.send(self.GenResponsMsg(response_message))
	                time.sleep(1)
	                response_message = f"""
...
"""
            ...
            if "1200" in data.hex()[0:4] and b"/slog" in data: 
	                json_result = get_available_room(data.hex()[10:])
	                parsed_data = json.loads(json_result)
	                uid = parsed_data["5"]["data"]["1"]["data"]
	                sender_id = parsed_data["5"]["data"]["1"]["data"]
...
"""
            ...
            if "1200" in data.hex()[0:4] and b"/admin" in data: 
	                ...
	                sender_name = parsed_data['5']['data']['9']['data']['1']['data']
	                response_message = f"""	                
[C][B]اول صانع بوتات في سلطنة عمان !

سمعت أنك تريد أن ترى من \'s المشرفين؟ 

أشعر وكأنك تريد شراء المسؤول أيضًا! 

أو على الأقل تريد شراء خادم باسمك 

صحيح؟ حسنًا، يمكنك ذلك! 

للتواصل فقط [00ff00]『σм』 ᵀᴱᴬᴹ![ffffff]

@vfk_6 , @kfv_77 , @3_b2w , @awad_aluka7

كما يمكنك الإرسال  [00ff00]/🇴🇲help [ffffff]لكي تتمكن من التعرف على أوامري!. 

بوت من صنع  [00ff00]『σм』 TAEMᵀᴱᴬᴹ
"""
	                clients.send(self.GenResponsMsg(response_message))   
	                
            if "1200" in data.hex()[0:4] and (
    b'\xd8\xba\xd8\xa8\xd9\x8a' in data or      # غبي
    b'\xd9\x83\xd9\x84\xd8\xa8' in data or      # كلب
    b'\xd8\xad\xd9\x85\xd8\xa7\xd8\xb1' in data or  # حمار
    b'\xd8\xaa\xd8\xa7\xd9\x81\xd9\x87' in data or  # تافه
    b'\xd8\xae\xd8\xb1\xd8\xa7' in data or      # خرا
    b'\xd9\x82\xd8\xb0\xd8\xb1' in data or      # قذر
    b'\xd9\x88\xd8\xb3\xd8\xae' in data or      # وسخ
    b'\xd8\xad\xd9\x8a\xd9\x88\xd8\xa7\xd9\x86' in data or  # حيوان
    b'\xd9\x82\xd8\xad\xd8\xa8\xd9\x87' in data or          # قحبه
    b'****' in data or                         # ****
    b'***' in data                             # ***
):
    # الكود الذي يرسل التحذير
	                json_result = get_available_room(data.hex()[10:])
	                parsed_data = json.loads(json_result)
	                uid = parsed_data["5"]["data"]["1"]["data"]
	                sender_id = parsed_data["5"]["data"]["1"]["data"]
	                sender_name = parsed_data['5']['data']['9']['data']['1']['data']
	                response_message = f"""	                
[C][B]⚠️ تم رصد رسالة تحتوي على ألفاظ غير لائقة!

[C][B]يرجى احترام الآخرين وعدم استخدام ألفاظ مسيئة.

[C][B]تم إرسال تحذير إلى: [00ff00]{sender_name}[ffffff]

[C][B]『σм』 TEAM[ffffff] لحماية المحادثات 👮‍♂️
"""
	                clients.send(self.GenResponsMsg(response_message))   
...
[C][B][FF0000] قيادات فريق 『σм』 TEAM:

[FF5555]• حسافه 『σм』 ⟹ [FF0000]المطور العام  
[808080]   إنستغرام: [FF4040]@vfk_6

[FF7777]• المعمري 『σм』 ⟹ [FF0000]المدير التنفيذي  
[808080]   إنستغرام: [FF4040]@kfv_77

[FF9999]• 『σм』That ⟹ [FF0000]نائب المدير  
[808080]   إنستغرام: [FF4040]@b2w_3

[FFA07A]• M3ASL 『σм』 ⟹ [FF0000]الدعم الفني  
[808080]   إنستغرام: [FF4040]@awad_aluka7

[C][B][FF0000]━━━━━━━━━━━━━

[C][B][FF0000]🤖 بوت رسمي مقدم من:
[FF3333]『σм』 ᵀᴱᴬᴹ

[808080]• جميع الحقوق محفوظة لفريق 『σм』
"""

	                clients.send(self.GenResponsMsg(response_message)) 
	                
            if "1200" in data.hex()[0:4] and b"/7SAFA" in data: 
	                json_result = get_available_room(data.hex()[10:])
	                parsed_data = json.loads(json_result)
	                uid = parsed_data["5"]["data"]["1"]["data"]
	                sender_id = parsed_data["5"]["data"]["1"]["data"]
...
"""
            ...
            if "1200" in data.hex()[0:4] and b"/vip" in data: 
	                json_result = get_available_room(data.hex()[10:])
	                parsed_data = json.loads(json_result)
	                uid = parsed_data["5"]["data"]["1"]["data"]
	                sender_id = parsed_data["5"]["data"]["1"]["data"]
...
[ffff00]-- أوامر المقبرة -- 



[ff3d00]لإضافة شخص للمقبرة : 

مثلا : 

[00ff00]/🇴🇲@1234🇴🇲56789

[ff3d00]لمسح شخص من المقبرة :

مثلا : 

[00ff00]-@12345🇴🇲6789

[ff3d00]لمعرفة قابر شخص معين 

مثلا : 

[00ff00]??

[ffff00]لإضافة شخص للحمايه

مثلا :

تنبيه لا تضع يوتيوبر في المقبره و إلا سيتم  حذفك من البوت 
\n\n[ffffff]---------------------------\n\n\n[2196f3]تابعني إنستا : \n\n[00ff00]instagram : @vfk_6
"""
            ...
            if "1200" in data.hex()[0:4] and b"/@" in data: 
	                ...
	                sender_name = parsed_data['5']['data']['9']['data']['1']['data']
	                response_message = f"""	                
[FF0000]تم قبر اللاعب لمدة 3 ساعات 

ملاحضه لا يمكنك حذف شخص من الامقبره بعد قبره
...
if __name__ == "__main__":
    try:
        client_thread = FF_CLIENT(id="3828884940", password="2DD465E620FBEF1A6FBBFFA889F42D6B9609D6927940EE84C368A3F14441EB23")
        client_thread.start()
    except Exception as e:
